...
// If it's not available, we'll need a fallback.
// For now, we'll try to import it and handle the case where it might be missing.
let SplitText;
try {
    // Try to import SplitText if available in the environment
    // In a real project, this would be installed via a private registry.
    // If not found, we'll implement a basic fallback animation.
} catch (e) {
    console.warn("GSAP SplitText not found, falling back to basic animations.");
}
 ...
 * @property {Array<{ text: string, href: string, primary?: boolean }>} [ctaButtons]
 * @property {Array<string>} [microDetails]
 */

/**
 * @param {HeroProps & { showBackground?: boolean }} props
 */
    ...
    useGSAP(
        () => {
            ...
            document.fonts.ready.then(() => {
                ...
                // If SplitText is not available, we animate the whole header
                if (SplitText) {
                    ...
                    // Fallback animation for the whole header
